...
DEFINE_double(explore_c, sqrt(3), "");
...
//    }
    ...
    std::string MCTSEngine::GetRootChooseStr() const {
        ...
        NodeInfo info[root_node_->move_node_size_];
        {
            for (int i = 0; i < root_node_->move_node_size_; i++) {
                ...
                info[i].win_rate = root_node_->move_node_[i].second->GetWinRate(root_node_->is_red);
            }
        }

        std::sort(info, info + n, [is_red](const NodeInfo &x, const NodeInfo &y) -> bool {
            return x.win_rate > y.win_rate;
        });
        const std::string space = "          ";
        std::stringstream ss;
        ss << "move" << space << "n" << space << "red_win" << space << "black win" << space << "win rate" << "\n";
        ss << std::fixed << std::setprecision(2);
        for (int i = 0; i < n; i++) {
            ss << info[i].move << space << info[i].n << space << info[i].red_win_count << space
               << info[i].black_win_count << space << info[i].win_rate << "\n";
        }
        return ss.str();
    }
    ...
    Node::Node(bool is_red, Node *father, const ChessBoard &board, const ChessMove &move)
            : is_red(is_red), n(0), black_win_count(0), red_win_count(0),
              ...
              move_node_size_(0), father(father), move_(move), is_repeat(false), end_(NOT_END), end_deep_(0) {
        if (father == nullptr) {
    ...
    BoardResult Node::ExpandTree(SearchCtx *ctx) {
        if (ctx->board.End() != BoardResult::NOT_END) {
            end_ = ctx->board.End();
            end_deep_ = 1; //avoid inf in get win rate
        }
        if (end_ != BoardResult::NOT_END) {
...
//        Xiang = 6,
    ...
    double Node::GetWinRate(bool is_red_rate) {
        int expect_end = is_red_rate ? RED_WIN : BLACK_WIN;
        if (end_ != BoardResult::NOT_END) {
            if (end_ == expect_end) {
                return 100 + 1.0 / end_deep_;
            } else {
                return 0 - 1.0 / end_deep_;
            }
        }
        double dw, dn;
        {
    ...
    std::pair<ChessMove, Node *> *Node::GetBestMove() {
        ...
        auto expect_end = is_red ? RED_WIN : BLACK_WIN;
        int expect_node_cnt = 0;
        int expect_min_end_deep = 1000;
        int unexpect_node_cnt = 0;
        int unexpect_max_end_deep = 0;
        std::pair<ChessMove, Node *> *unexpect_ptr, *expect_ptr = nullptr;

        for (int i = 0; i < move_node_size_; i++) {
            if (move_node_[i].second == nullptr ||
                move_node_[i].second->n.load(std::memory_order::memory_order_relaxed) == 0)
                continue;
            auto end = move_node_[i].second->end_;
            int end_deep = move_node_[i].second->end_deep_ + 1;
            if (end != BoardResult::NOT_END) {
                if (end == expect_end) {
                    expect_node_cnt++;
                    if (end_deep < expect_min_end_deep) {
                        expect_min_end_deep = end_deep;
                        expect_ptr = move_node_ + i;
                    }
                } else { //unexpect
                    unexpect_node_cnt++;
                    if (end_deep > unexpect_max_end_deep) {
                        unexpect_max_end_deep = end_deep;
                        unexpect_ptr = move_node_ + i;
                    }
                    continue;
                }
            }
        ...
        if (expect_node_cnt > 0) {
            assert(expect_min_end_deep < 1000);
            end_ = expect_end;
            end_deep_ = expect_min_end_deep;
            assert(expect_ptr != nullptr);
            return expect_ptr;
        }
        if (unexpect_node_cnt == move_node_size_) {
            assert(unexpect_max_end_deep > 0);
            end_ = expect_end == RED_WIN ? BLACK_WIN : RED_WIN;
            end_deep_ = unexpect_max_end_deep;
            assert(unexpect_ptr != nullptr);
            return unexpect_ptr;
        }
        return ptr;
    }
